    ...
    <div ref={dropdownRef} className="flex w-full flex-col gap-4 pt-5">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        ...
        <Button
          ...
          icon={<SiDogecoin className="h-6 w-6" />}
          variant={
      ...
      <div className="relative flex w-full space-y-3">
        <input
          ...
          className={`flex w-full rounded-xl border-1 border-[#222222] text-center ${
            !['Bitcoin', 'Litecoin', 'Dogecoin'].includes(
              selectedCurrencyName || ''
            )
              ? 'bg-[#222222] text-[#ffffff]'
              : 'bg-[#f0f0f0] text-[#222222]'
          } p-2 font-space-grotesk text-[16px]`}
          style={{
            color:
              !['Bitcoin', 'Litecoin', 'Dogecoin'].includes(
                selectedCurrencyName || ''
              ) || (isFocused && selectedCurrencyName && !['Bitcoin', 'Litecoin', 'Dogecoin'].includes(selectedCurrencyName))
                ? '#ffffff'
                : '#222222',
          }}
        ...
        <div
          role="button"
          tabIndex={0}
          className="absolute right-3 inset-y-0 flex items-center cursor-pointer focus:outline-none"
          onClick={() => setShowDropdown((prev) => !prev)}
          onKeyDown={(e) => {
