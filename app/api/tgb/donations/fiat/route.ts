...
import type { DonationPledge, FiatDonationResponse } from '@/types/donation'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await client.post<{ data: FiatDonationResponse }>(
      '/donation/fiat',
      {
    ...
    console.error('Error creating fiat donation:', error)
    return NextResponse.json(
      {
        error: 'Failed to create fiat donation',
